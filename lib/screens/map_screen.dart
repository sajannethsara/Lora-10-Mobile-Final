...
//               styleUri: mapService.tileStore != null
...
// =======================================================================================
// Fixed MapScreen - Properly listens to MapService changes
// =======================================================================================
...
class _MapScreenState extends State<MapScreen> {
  ...
  mapbox.PolylineAnnotation? _pathAnnotation;
  List<mapbox.Point> _pathPoints = [];
  String? _lastAreaKey; // Track the last area to detect changes
  ...
  Future<void> _moveCamera(mapbox.Point point, {double? zoom}) async {
    if (_mapController == null) return;
    ...
    try {
      ...
      await _mapController!.easeTo(
        mapbox.CameraOptions(
          center: point,
          zoom: zoom ?? currentCamera.zoom,
          bearing: currentCamera.bearing,
          pitch: currentCamera.pitch,
        ),
        mapbox.MapAnimationOptions(duration: 1000), // Longer duration for area changes
      );
    } catch (e) {
  ...
  // Handle area change - move to new area center
  void _handleAreaChange(String newAreaKey, mapbox.Point newCenter) {
    if (_lastAreaKey != newAreaKey) {
      _lastAreaKey = newAreaKey;
      print('Area changed to: $newAreaKey');
      _updatePathFromProvider(); // Ensure path points are updated for new area
      _moveCamera(newCenter, zoom: 12.0);
      _updateAnnotations(); // Update annotations for new area
    }
  }

  @override
  Widget build(BuildContext context) {
    final provider = Consumer<AppProvider>(
      builder: (context, appProvider, child) {
        // Update path when gpsBucket changes
        _updatePathFromProvider();
        if (_mapController != null && _pathPoints.isNotEmpty) {
          _updateAnnotations();
          // Smoothly follow the latest GPS point
          _moveCamera(_pathPoints.last);
        }
    ...
    return Consumer<MapService>(
      builder: (context, mapService, child) {
        ...
        // Handle area changes
        if (currentAreaKey != null && mapCenter != null) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            _handleAreaChange(currentAreaKey, mapCenter);
          });
        }

        // Determine initial map center
        final initialCenter = mapCenter ??
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(currentAreaKey != null 
                ? (mapService.getAreaInfo(currentAreaKey)?['name'] ?? 'Map')
                : 'Map'),
            actions: [
              if (currentAreaKey != null)
                IconButton(
                  ...
                  onPressed: () {
                    if (mapCenter != null) {
                      _moveCamera(mapCenter, zoom: 12.0);
                    }
                  },
                ),
            ],
          ...
          body: mapbox.MapWidget(
            key: ValueKey('mapWidget-$currentAreaKey'), // Force rebuild on area change
            ...
            onMapCreated: (controller) async {
              ...
              _updatePathFromProvider(); // Ensure path points are updated after map creation
              await _updateAnnotations();
              print('Map created for area: $currentAreaKey');
            },
          ),
        );
      },
